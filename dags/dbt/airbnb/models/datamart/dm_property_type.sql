...
WITH
  ...
  ,agg_data AS
    (SELECT
      ...
      accommodates,
      month_year,
      COUNT(DISTINCT active_listing_id)*100/ COUNT(DISTINCT listing_id)::FLOAT AS active_listing_rate,
      ...
      (COUNT(DISTINCT CASE WHEN host_is_superhost = 't' THEN host_id END)::FLOAT*100/COUNT(DISTINCT host_id))::FLOAT AS super_host_rate,
      AVG(CASE WHEN {{ is_active_listing }} THEN review_scores_rating END)::FLOAT AS active_listing_avg_review_scores_rating,
      SUM(number_of_stays) AS total_number_of_stays,
      (
        SUM(number_of_stays * active_listing_price)
        / COUNT(DISTINCT active_listing_id)
      )::FLOAT AS avg_estimated_revenue_per_active_listing,
  ...
  SELECT
    ...
    (
      (unique_active_listing_count - previous_month_unique_active_listing_count)::FLOAT*100
      / NULLIF(previous_month_unique_active_listing_count, 0)
    )::FLOAT AS active_listings_percentage_change,
    (
      (unique_inactive_listing_count - previous_month_unique_inactive_listing_count)::FLOAT*100
